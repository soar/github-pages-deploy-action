...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        const folderPath = util_1.generateFolderPath(action, 'folder');
        const rootPath = util_1.generateFolderPath(action, 'root');
        ...
        try {
            ...
            yield execute_1.execute(`rsync -q -av --checksum --progress ${folderPath}/. ${action.targetFolder
                ? `${temporaryDeploymentDirectory}/${action.targetFolder}`
                : temporaryDeploymentDirectory} ${action.clean
                ? `--delete ${excludes} ${!fs_1.default.existsSync(`${folderPath}/CNAME`) ? '--exclude CNAME' : ''} ${!fs_1.default.existsSync(`${folderPath}/.nojekyll`)
                    ? '--exclude .nojekyll'
                    : ''}`
                : ''}  --exclude .ssh --exclude .git --exclude .github ${folderPath === rootPath
